...
from models import LegResult, NavPoint
from core.formatting import format_minutes_mmss
from core.geo import interpolate_line


def determine_leg_end_target_alt(
    leg: LegResult, verticale_ft: float, tdp_ft: float
...
def compute_leg_vertical_segments(
    ...
    if delta_climb_ft > 1:
        ...
        climb_dist_nm = climb_time_min = 0.0
    ...
    if delta_descent_ft > 1:
        ...
        descent_dist_nm = descent_time_min = 0.0
...
def build_vertical_profile(
    ...
    for i, leg in enumerate(legs):
        ...
        is_arrival_aerodrome, terrain_alt, end_target_alt = determine_leg_end_target_alt(
            leg, verticale_ft, tdp_ft
        )
        vs = compute_leg_vertical_segments(
        ...
        toc_nm_local = vs["toc_nm_local"]
        tod_nm_local = vs["tod_nm_local"]

        leg_start_elapsed_min = elapsed_min_total
        if toc_nm_local is not None:
            toc_x = round(cumulative_nm + toc_nm_local, 1)
            toc_t = format_minutes_mmss(leg_start_elapsed_min + vs["climb_time_min"])
            toc_marks.append((toc_x, toc_t))

        if tod_nm_local is not None:
            cruise_nm = max(tod_nm_local - max(vs["climb_dist_nm"], 0.0), 0.0)
            cruise_t  = cruise_nm / max(leg.gs_kt, 1e-6) * 60.0
            tod_x = round(cumulative_nm + tod_nm_local, 1)
            tod_t = format_minutes_mmss(leg_start_elapsed_min + vs["climb_time_min"] + cruise_t)
        ...
        if is_arrival_aerodrome:
            if leg.end_type == "verticale":
                vt_marks.append((leg_end_x, terrain_alt, terrain_alt + verticale_ft, leg.end_name))
            elif leg.end_type == "tour_de_piste":
                tdp_marks.append((leg_end_x, terrain_alt, terrain_alt + tdp_ft, leg.end_name))
        ...
        for j, (pt, x_local) in enumerate(zip(seg_pts, seg_x_local)):
            ...
            alt = altitude_at_leg_distance(
                ...
                cruise_alt=leg.altitude_ft,
                end_target_alt=end_target_alt,
                climb_dist_nm=vs["climb_dist_nm"],
                descent_dist_nm=vs["descent_dist_nm"],
