...
from models import LegInput, END_TYPES, LEG_TYPES
from core.formatting import route3, format_minutes_mmss, correction_label, m_to_ft
from core.navigation import build_route, generation_hour_utc
...
with st.expander("Terrain de départ", expanded=True):
    ...
    st.markdown("### Météo")
    weather_block = f"""
    <div style="background-color:rgba(255,255,255,0.03);padding:14px;border-radius:10px;
        border:1px solid rgba(255,255,255,0.08);font-family:monospace;white-space:pre-wrap;line-height:1.4;">
    <b>METAR</b>
    {html_lib.escape(metar_raw) if metar_raw else "METAR indisponible."}

    <br><b>TAF</b>
    {html_lib.escape(taf_raw) if taf_raw else "TAF indisponible."}
        </div>"""
        ...
        with t1:
            ...
            if leg["leg_type"] == "point_tournant":
                leg["route_true_deg"] = st.number_input(
                    "Route vraie (°)", min_value=0, max_value=359,
                    value=int(round(leg["route_true_deg"])) % 360, step=1,
                    key=f"route_{i}",
                )
                leg["distance_nm"] = st.number_input(
                    "Distance (NM)", min_value=0.1, max_value=500.0,
                    value=float(leg["distance_nm"]), step=1.0,
                    key=f"dist_{i}",
                )
                leg["label"] = st.text_input("Label", value=leg["label"], key=f"label_{i}")
                st.caption(f"RV affichée : {route3(leg['route_true_deg'])}")
            else:
                leg["target_icao"] = st.text_input(
                    "OACI arrivée", value=leg["target_icao"], key=f"icao_{i}",
                ).strip().upper()

        with t2:
            leg["altitude_ft"] = st.number_input(
                "Altitude branche (ft)", min_value=500, max_value=18000,
                value=int(leg["altitude_ft"]), step=100, key=f"alt_{i}",
            )
            leg["end_type"] = st.selectbox(
                "Fin de branche", END_TYPES,
...
route_key = (dep_icao, round(float(tas_kt), 1), metar_sig, legs_signature(st.session_state.legs_data))
...
if st.session_state.get("route_key") == route_key:
    ...
    try:
        legs, nav_points = build_route(departure, legs_in, tas_kt, departure_metar_decoded=metar_decoded)
...
with tabs[0]:
    ...
    st.selectbox(
        "Fond de carte", basemap_options,
        index=basemap_options.index(st.session_state.basemap_choice),
        key="basemap_selector", on_change=sync_basemap_choice, width="stretch",
    )
...
with tabs[1]:
    ...
    with c1: metric_card("Distance totale", f"{total_nm:.1f} NM")
    with c2: metric_card("Temps total", f"{total_min:.1f} min")
    with c3: metric_card("Trip fuel", f"{trip_fuel_l:.1f} L")
    with c4: metric_card("Fuel + réserve", f"{total_fuel_l:.1f} L")
...
with tabs[2]:
    ...
    profile_key = (st.session_state["route_key"], climb_rate_fpm, climb_speed_kt, descent_rate_fpm, verticale_ft, tdp_ft)
    if st.session_state.get("profile_key") == profile_key:
        ...
        profile = build_vertical_profile(
            nav_points=nav_points, legs=legs,
            climb_rate_fpm=climb_rate_fpm, climb_speed_kt=climb_speed_kt,
            descent_rate_fpm=descent_rate_fpm,
            verticale_ft=verticale_ft, tdp_ft=tdp_ft,
        )
    ...
    terrain_ft = [int(round(m_to_ft(x))) for x in elev_m] if elev_m else [0] * len(profile["terrain_route_pts"])
    if not elev_m:
    ...
    fig.add_trace(go.Scatter(
        x=x_terrain, y=terrain_ft, mode="lines", name="Sol",
        fill="tozeroy", line=dict(color="#8B5A2B", width=2),
        fillcolor="rgba(139, 90, 43, 0.45)",
        hovertemplate="Dist %{x:.1f} NM<br>Sol %{y:.0f} ft<extra></extra>",
    ))
    fig.add_trace(go.Scatter(
        x=x_air, y=y_air, mode="lines", name="Avion",
        line=dict(width=3), connectgaps=False,
        hovertemplate="Dist %{x:.1f} NM<br>Avion %{y:.0f} ft<extra></extra>",
    ))

    y_max_valid = max(y for y in y_air if y is not None)
    for x, t_txt in profile["toc_marks"]:
        fig.add_annotation(x=round(x, 1), y=y_max_valid, text=f"TOC {t_txt}", showarrow=False, yshift=10, font=dict(color="green"))
    for x, t_txt in profile["tod_marks"]:
        fig.add_annotation(x=round(x, 1), y=y_max_valid, text=f"TOD {t_txt}", showarrow=False, yshift=10, font=dict(color="purple"))

    for x, y0, y1, terrain_name in profile["vt_marks"]:
        fig.add_shape(type="line", x0=round(x,1), x1=round(x,1), y0=round(y0), y1=round(y1), line=dict(color="orange", width=2, dash="dot"))
        fig.add_annotation(x=round(x,1), y=round(y1), text=f"VT<br>{terrain_name} {round(y0):.0f} ft", showarrow=False, yshift=10, font=dict(color="orange"), align="center")

    for x, y0, y1, terrain_name in profile["tdp_marks"]:
        fig.add_shape(type="line", x0=round(x,1), x1=round(x,1), y0=round(y0), y1=round(y1), line=dict(color="deepskyblue", width=2, dash="dot"))
        fig.add_annotation(x=round(x,1), y=round(y1), text=f"TDP<br>{terrain_name} {round(y0):.0f} ft", showarrow=False, yshift=10, font=dict(color="deepskyblue"), align="center")

    fig.update_layout(
        height=430, margin=dict(l=20, r=20, t=20, b=20),
        xaxis_title="Distance cumulée (NM)", yaxis_title="Altitude (ft)",
    ...
    if x_terrain:
        c1, c2, c3 = st.columns(3)
        with c1: metric_card("Distance totale", f"{x_terrain[-1]:.1f} NM")
        with c2: metric_card("Alt min avion",   f"{min(y for y in y_air if y is not None):.0f} ft")
        with c3: metric_card("Alt max avion",   f"{max(y for y in y_air if y is not None):.0f} ft")
    ...
    if terrain_ft and len(x_terrain) == len(terrain_ft):
        ...
        margins     = [y - terrain_map[round(x, 1)] for x, y in air_pairs if round(x, 1) in terrain_map]
        if margins:
