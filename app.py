...
# Rayon déroutement (NM)
DIVERT_RADIUS_NM = 20.0

# ─── PAGE ───
st.set_page_config(page_title="SkyAssistant V54.2", layout="wide")
...
def get_http_session() -> requests.Session:
    s = requests.Session()
    s.headers.update({"User-Agent": "SkyAssistant/54.2"})
    return s
...
with col_map:
    if st.session_state.waypoints:
        ...
        st_folium(m, width="100%", height=300, key="map_v54_2", returned_objects=[])
