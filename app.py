...
# Mapping de pression optimisé pour AROME (1 hPa ≈ 28 ft)
# On cible les niveaux exacts pour éviter les erreurs d'interpolation
PRESSURE_MAP = {
    1000: 975, 1500: 960, 2000: 950, 2500: 925, 3000: 900, 
    3500: 885, 4500: 865, 5000: 850, 5500: 835, 7500: 775, 9500: 725
}

@st.cache_data(ttl=3600) # On réduit le cache pour coller aux runs AROME
def get_wind_arome(lat, lon, alt_ft, time_dt):
    # Trouver le palier de pression le plus proche
    target_alt = min(PRESSURE_MAP.keys(), key=lambda x: abs(x - alt_ft))
    lv = PRESSURE_MAP[target_alt]
    
    params = {
        "latitude": lat, "longitude": lon,
        "hourly": f"wind_speed_{lv}hPa,wind_direction_{lv}hPa",
        "models": "meteofrance_arome_france_hd", # Le modèle de Windy
    ...
    try:
        ...
        if "hourly" not in r or r["hourly"][f"wind_speed_{lv}hPa"][0] is None:
            # Fallback automatique sur le modèle global si AROME HD est aux fraises
...
# ─── INTERFACE ───
st.set_page_config(page_title="SkyAssistant V15 - AROME HD", layout="wide")
if 'waypoints' not in st.session_state: st.session_state.waypoints = []

with st.sidebar:
    st.title("🛠️ Paramètres")
    oaci = st.text_input("🔍 Code OACI Départ", "").upper()
    # Chargement aéroport simplifié pour l'exemple
    if oaci == "LFMW": 
        ap = {"name": "Castelnaudary", "lat": 43.3086, "lon": 1.9161}
        st.success(f"📍 {ap['name']}")
...
with col_ctrl:
    ...
    if st.button("➕ Ajouter") and st.session_state.waypoints:
        ...
        st.session_state.waypoints.append({"name": f"WP{len(st.session_state.waypoints)}", "lat": round(n_lat, 4), "lon": round(n_lon, 4), "tc": tc_in, "dist": dist_in, "alt": alt_in})
        st.rerun()
...
with col_map:
    if st.session_state.waypoints:
        ...
        st_folium(m, width="100%", height=350, key="map_v15", returned_objects=[])
...
if len(st.session_state.waypoints) > 1:
    ...
    for i in range(1, len(st.session_state.waypoints)):
        ...
        wd, ws = get_wind_arome(w2["lat"], w2["lon"], w2["alt"], curr_t)
    ...
    st.subheader("📋 Log de Navigation (Source AROME HD)")
