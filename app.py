...
# Mapping de pression précis (1 hPa ≈ 28 ft)
PRESSURE_MAP = {
...
def load_airports():
    try:
        # Chargement de la base de données des aéroports français
...
# ─── INTERFACE STREAMLIT ───
st.set_page_config(page_title="SkyAssistant V18", layout="wide")
...
with st.sidebar:
    ...
    if st.button("🗑️ Reset Plan de Vol", key="reset_btn"):
...
with col_ctrl:
    st.subheader("📍 Gestion des Branches")
...
with col_map:
    if st.session_state.waypoints:
        ...
        m = folium.Map(location=[start["lat"], start["lon"]], zoom_start=9)
        ...
        st_folium(m, width="100%", height=350, key="map_v18", returned_objects=[])
    else:
        st.info("Saisissez un code OACI (ex: LFBI) dans la barre latérale.")
...
if len(st.session_state.waypoints) > 1:
    ...
    st.success(f"**Distance : {t_dist:.1f} NM | Temps : {int(t_min)} min | Fuel (+10L) : {fuel} L**")
    st.caption(f"Source Météo : AROME HD | Heure actuelle : {curr_t.strftime('%H:%M')} UTC")
