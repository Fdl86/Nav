...
# ─── API HELPERS (optimisés + timeouts) ───
...
with st.sidebar:
    ...
    fuel_flow = st.number_input("Conso (L/h)", min_value=5.0, max_value=100.0, value=25.0, step=0.1)
...
with col_ctrl:
    ...
    if st.button("➕ Ajouter") and st.session_state.waypoints:
        ...
        new_wp = Waypoint(
            ...
            lat=la2, lon=lo2, tc=tc_in, dist=dist_in, alt=alt_in,
            manual_wind=m_wind, elev=get_elevation_ft(la2, lo2), arr_type="Direct"
        )
...
# ─── LOG + PROFIL (calculs uniquement quand nécessaire) ───
    ...
    for i in range(1, len(st.session_state.waypoints)):
        ...
        if at != "Direct":
            ...
            fig.add_annotation(x=d_total + w2.dist, y=alt_t,
                               text=f"<b>{label_dest} {w2.name}</b>", showarrow=False, yshift=15,
