import os, hashlib, json, threading, math, re
...
MAP_HTML_PATH  = "/tmp/skyassistant_map.html"
MAP_SERVER_PORT = 8765

# ─── MINI SERVEUR HTTP pour la carte ───
@st.cache_resource
def start_map_server():
    ...
    try:
        server = HTTPServer(("localhost", MAP_SERVER_PORT), Handler)
        t = threading.Thread(target=server.serve_forever, daemon=True)
        t.start()
        return True
    except OSError:
        return True  # port déjà ouvert = déjà lancé

start_map_server()

# ─── PAGE ───
st.set_page_config(page_title="SkyAssistant V58.8", layout="wide")
...
def get_http_session():
    s = requests.Session()
    s.headers.update({"User-Agent":"SkyAssistant/58.8"})
    return s
SESSION = get_http_session()

# ─── STATE ───
for k,v in [("waypoints",[]),("wx_refresh",0),("map_hash","")]:
...
def write_map_file(waypoints):
    """Génère le HTML Folium et l'écrit dans /tmp. Ne régénère que si les waypoints ont changé."""
    new_hash = waypoints_hash(waypoints)
    if st.session_state.map_hash == new_hash and os.path.exists(MAP_HTML_PATH):
        return  # rien à faire

    center=[waypoints[0]["lat"],waypoints[0]["lon"]]
    m=folium.Map(location=center,zoom_start=9,control_scale=True,tiles=None)
    folium.TileLayer("openstreetmap",name="🗺️ Standard",overlay=False,control=True,show=True).add_to(m)
    folium.TileLayer(
        tiles="https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}",
        attr="Google Satellite",name="🛰️ Satellite",overlay=False,control=True,show=False).add_to(m)
    if OPENAIP_API_KEY:
        folium.TileLayer(
            tiles=f"https://api.tiles.openaip.net/api/data/openaip/{{z}}/{{x}}/{{y}}.png?apiKey={OPENAIP_API_KEY}",
            attr="openAIP",name="✈️ Aviation (openAIP)",overlay=False,control=True,show=False).add_to(m)
    ...
    st.session_state.map_hash = new_hash

def map_iframe():
    """Affiche la carte via une iframe pointant sur le serveur local.
    La src inclut le hash pour forcer le rechargement UNIQUEMENT quand le tracé change."""
    h = st.session_state.map_hash
    url = f"http://localhost:{MAP_SERVER_PORT}/skyassistant_map.html?v={h}"
    iframe_html = f"""
    <iframe
        src="{url}"
    ...
    ></iframe>
    """
    components.html(iframe_html, height=430, scrolling=False)

# ══════════════════════════════════════════
#  SIDEBAR
# ══════════════════════════════════════════
with st.sidebar:
    st.title("✈️ SkyAssistant V58.8")
    ...
    if st.button("🗑️ Reset",use_container_width=True):
        st.session_state.waypoints=[]; st.session_state.map_hash=""; st.rerun()
...
with col_map:
    if st.session_state.waypoints:
        write_map_file(st.session_state.waypoints)  # écrit /tmp/skyassistant_map.html si besoin
        map_iframe()                                  # affiche via iframe → stable entre reruns
