...
# ─── API HELPERS ───
@st.cache_data(ttl=1800)  # 30 min (élévation ne change jamais)
...
@st.cache_data(ttl=300)  # 5 min de cache sur la météo
def get_wind_v27_final(lat: float, lon: float, alt_ft: int, target_time: dt.datetime, manual_wind: Optional[Dict] = None) -> Tuple[int, int, str]:
    if manual_wind:
...
with st.sidebar:
    ...
    if sugg and st.button(f"Départ : {sugg[0]}"):
        ...
        st.session_state.waypoints = [Waypoint(
            name=sugg[0], lat=ap['lat'], lon=ap['lon'], alt=elev, elev=elev, arr_type="Direct"
        )]
        st.rerun()
...
with col_map:
    if st.session_state.waypoints:
        ...
        folium.TileLayer(tiles='https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', attr='Google Satellite',
                         name='Vue Satellite', overlay=False, control=True).add_to(m)
        ...
        for i, w in enumerate(st.session_state.waypoints):
            color = "blue" if i == 0 else ("red" if i == num_w - 1 else "orange")
            icon = "plane" if i == 0 else ("flag" if i == num_w - 1 else "dot-circle-o")
            folium.Marker(
                [w.lat, w.lon],
                popup=w.name,
...
if len(st.session_state.waypoints) > 1:
    ...
    for i in range(1, len(st.session_state.waypoints)):
        ...
        wd, ws, src = get_wind_v27_final(w2.lat, w2.lon, w2.alt, curr_t, w2.manual_wind)
            ...
            if d_climb > 0.1:
                ...
                if d_climb < w2.dist:
                    ...
                    fig.add_annotation(x=d_total + d_climb, y=alt_crois,
                                       text=f"TOC {round(d_climb, 1)}NM ({t_cl_str})", showarrow=True, ay=45)
        ...
        if at != "Direct":
            ...
            if d_desc > 0.1:
                ...
                if d_desc < w2.dist:
                    ...
                    fig.add_annotation(x=d_total + (w2.dist - d_desc), y=alt_crois,
                                       text=f"TOD {round(d_desc, 1)}NM ({t_de_str})", showarrow=True, ay=-45)

            label_dest = "VT" if "VT" in at else "TDP"
            fig.add_annotation(x=d_total + w2.dist, y=alt_t,
    ...
    if not edited_log.equals(df_nav):
        ...
        for _, row in edited_log.iterrows():
            if not row['❌']:
                wp = st.session_state.waypoints[row['_idx']].__dict__.copy()
                if "➔" in row['Branche']:
                    wp['name'] = row['Branche'].split("➔")[1]
                wp['arr_type'] = row['Arrivée']
                new_wps.append(Waypoint(**wp))
    ...
    st.markdown('<div style="overflow-x: auto; width: 100%; border: 1px solid #444; border-radius: 10px;">',
